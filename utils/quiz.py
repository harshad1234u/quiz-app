...
def get_recommended_questions(user_id: int, count: int = 10) -> list[dict]:
    ...
    # Try selected_topics from users table first (new JSONB column)
    interest_names = []
    try:
        user_result = sb.table("users").select("selected_topics").eq("user_id", user_id).execute()
        if user_result.data:
            topics = user_result.data[0].get("selected_topics")
            if topics and isinstance(topics, list):
                interest_names = topics
            elif topics and isinstance(topics, str):
                import json as _json
                interest_names = _json.loads(topics)
    except Exception:
        pass
